...
class SandboxTimeoutError(SandboxNetworkError):
    ...
    """
    def __init__(self, message: str, timeout_ms: Optional[int] = None, operation: Optional[str] = None):
        super().__init__(message)
...
class Sandbox:
    ...
    def __init__(self, sandbox_id: str, name: str, base_url: str, api_key: Optional[str] = None):
        ...
        headers = {
            "User-Agent": "concave-sandbox/0.1.0",
    ...
    @classmethod
    def create(cls, name: str, base_url: Optional[str] = None, api_key: Optional[str] = None) -> 'Sandbox':
        """
        ...
        try:
            ...
            if "id" not in sandbox_data:
                raise SandboxInvalidResponseError(f"Invalid response from sandbox service: {sandbox_data}")
            ...
            raise SandboxTimeoutError(f"Sandbox creation timed out", timeout_ms=30000, operation="create") from e
    ...
    def execute(self, command: str, timeout: Optional[int] = None) -> ExecuteResult:
        ...
        try:
            ...
            if status_code == 404:
                ...
                raise SandboxAuthenticationError(f"Authentication failed") from e
            elif status_code == 429:
                raise SandboxRateLimitError(f"Rate limit exceeded") from e
            elif status_code == 500:
                raise SandboxInternalError(f"Server error during command execution") from e
            elif status_code == 502 or status_code == 503:
                raise SandboxUnavailableError(f"Sandbox {self.sandbox_id} is not ready or unreachable", status_code) from e
            else:
                error_msg = f"HTTP {status_code}"
                try:
                    error_data = e.response.json()
            ...
            raise SandboxTimeoutError(f"Command execution timed out", timeout_ms=timeout_val, operation="execute") from e
        except httpx.RequestError as e:
            raise SandboxConnectionError(f"Failed to connect to sandbox service: {e}") from e
    ...
    def run(self, code: str, timeout: Optional[int] = None) -> RunResult:
        ...
        try:
            ...
            if status_code == 404:
                ...
                raise SandboxInternalError(f"Server error during Python execution") from e
            elif status_code == 502 or status_code == 503:
                raise SandboxUnavailableError(f"Sandbox {self.sandbox_id} is not ready or unreachable", status_code) from e
            else:
                error_msg = f"HTTP {status_code}"
            ...
            timeout_val = timeout if timeout else 10000
            raise SandboxTimeoutError(f"Python execution timed out", timeout_ms=timeout_val, operation="run") from e
        except httpx.RequestError as e:
            raise SandboxConnectionError(f"Failed to connect to sandbox service: {e}") from e
    ...
    def delete(self) -> bool:
        ...
        try:
            response = self._client.delete(
                f"{self._sandboxes_url}/{self.sandbox_id}"
            )
            response.raise_for_status()
            data = response.json()
    ...
    def status(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if status_code == 404:
                ...
                raise SandboxInternalError(f"Server error while fetching status") from e
            elif status_code == 502 or status_code == 503:
                raise SandboxUnavailableError(f"Service unavailable", status_code) from e
            else:
                error_msg = f"HTTP {status_code}"
                try:
                    error_data = e.response.json()
            ...
            raise SandboxTimeoutError(f"Status check timed out", timeout_ms=5000, operation="status") from e
        except httpx.RequestError as e:
            raise SandboxConnectionError(f"Failed to connect to sandbox service: {e}") from e
